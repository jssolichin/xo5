    ...
    if (typeof a.XList == "undefined") {
        ...
        a.Suggest = Class.create({options: {minchars: 1,method: "get",varname: "input",className: "ajaxsuggest",timeout: 2500,delay: 500,offsety: 0,shownoresults: true,noresults: "No results!",maxheight: 250,cache: false,seps: "",icon: null,resultsParameter: "results",resultId: "id",resultValue: "value",resultInfo: "info",resultIcon: "icon",resultHint: "hint",parentContainer: "body",highlight: true,fadeOnClear: true,insertBeforeSuggestions: null,displayValue: false,displayValueText: "Value :",align: "left",unifiedLoader: false,loaderNode: null},sInput: "",nInputChars: 0,aSuggestions: [],iHighlighted: null,isActive: false,initialize: function(c, d) {
                ...
                if (g.length > this.nInputChars && this.aSuggestions.length && this.options.cache) {
                    ...
                    d++) {
                        if (this.aSuggestions[d].value.substr(0, g.length).toLowerCase() == g.toLowerCase()) {
                ...
                e++) {
                    ...
                    var c = new Ajax.Request(d, {method: j,requestHeaders: h,onSuccess: this.setSuggestions.bindAsEventListener(this, f, g),onFailure: function(i) {
                            new b.widgets.Notification("Failed to retrieve suggestions : ')" + i.statusText, "error", {timeout: 5})
                        }})
                }
            },setSuggestions: function(f, g, j) {
                if (j < this.latestRequest) {
                    return
                }
                this.aSuggestions = [];
                if (g.json) {
                    var h = f.responseJSON;
                    if (!h) {
                        return false
                    }
                    ...
                    d < e.length; 
                    d++) {
                        this.aSuggestions.push({id: e[d][g.resultId || this.options.resultId],value: e[d][g.resultValue || this.options.resultValue],info: e[d][g.resultInfo || this.options.resultInfo],icon: e[d][g.resultIcon || this.options.resultIcon],hint: e[d][g.resultHint || this.options.resultHint]})
                    }
                } else {
                    var c = f.responseXML;
                    ...
                    d < e.length; 
                    d++) {
                        if (e[d].hasChildNodes()) {
                            this.aSuggestions.push({id: e[d].getAttribute("id"),value: e[d].childNodes[0].nodeValue,info: e[d].getAttribute("info"),icon: e[d].getAttribute("icon"),hint: e[d].getAttribute("hint")})
                        }
                    }
                }
                ...
                if (!$(this.options.parentContainer).down(".suggestItems")) {
                    ...
                    if (this.options.align == "left") {
                        e.style.left = k.left + "px"
                    } else {
                        if (this.options.align == "center") {
                            e.style.left = k.left + (this.fld.getWidth() - l - 2) / 2 + "px"
                        } else {
                            e.style.left = (k.left - l + this.fld.offsetWidth - 2) + "px"
                        }
                    }
                    ...
                    document.getElementById('xo-content').insert({'before':e});
                    this.container = e;
                ...
                if (this.sources.length > 1) {
                    ...
                    g++) {
                        ...
                        if (this.resultContainer.down(".results" + d.id)) {
                            ...
                            var n = new Element("div", {"class": "results results" + d.id}), m = new Element("div", {"class": "sourceName"});
                            if (this.options.unifiedLoader) {
                                n.addClassName("hidden loading")
                            }
            ...
            },createList: function(h, d) {
                this.isActive = true;
                var c = this;
                this.killTimeout();
                if (this.sources.length > 1) {
                    var e = this.resultContainer.down(".results" + d.id);
                    if (h.length > 0 || this.options.shownoresults) {
                        e.down(".sourceContent").removeClassName("loading");
                ...
                var k = new b.widgets.XList([], {icon: this.options.icon,classes: "suggestList",eventListeners: {click: function() {
                ...
                g < j; 
                g++) {
                    var f = d.highlight ? this.emphasizeMatches(this.sInput, h[g].value) : h[g].value;
                    if (h[g].hint) {
                        f += "<span class='hint'>" + h[g].hint + "</span>"
                    }
                    if (!this.options.displayValue) {
                        var o = new Element("span", {"class": "info"}).update(f)
                    } else {
                        var o = new Element("div").insert(new Element("div", {"class": "value"}).update(f)).insert(new Element("div", {"class": "info"}).update("<span class='legend'>" + this.options.displayValueText + "</span>" + h[g].info))
                    }
                    if (h[g].icon) {
                        var l = new Element("img", {src: h[g].icon,"class": "icon"});
                        o.insert({top: l})
                    }
                ...
                if (h.length == 0) {
                    k.addItem(new b.widgets.XListItem(this.options.noresults, {classes: "noSuggestion",noHighlight: true}))
                }
                ...
                if (this.iHighlighted && !this.iHighlighted.hasClassName("noSuggestion")) {
                    ...
                    if (this.sInput == "" && this.fld.value == "") {
                        g = e = this.iHighlighted.down(".suggestValue").innerHTML
                    } else {
                        if (this.seps) {
                            var j = -1;
                            for (var f = 0; 
                            f < this.seps.length; 
                            f++) {
                                if (this.fld.value.lastIndexOf(this.seps.charAt(f)) > j) {
                                    j = this.fld.value.lastIndexOf(this.seps.charAt(f))
                                }
                            }
                            if (j == -1) {
                                g = e = this.iHighlighted.down(".suggestValue").innerHTML
                            } else {
                                e = this.fld.value.substring(0, j + 1) + this.iHighlighted.down(".suggestValue").innerHTML;
                                g = e.substring(j + 1)
                            }
                        } else {
                            g = e = this.iHighlighted.down(".suggestValue").innerHTML
                    ...
                    if (!h.stopped) {
                        ...
                        if (typeof (this.options.callback) == "function") {
                            this.options.callback({id: this.iHighlighted.down(".suggestId").innerHTML,value: this.iHighlighted.down(".suggestValue").innerHTML,info: this.iHighlighted.down(".suggestInfo").innerHTML})
                        }
                        if (this.fld.id.indexOf("_suggest") > 0) {
                            var d = this.fld.id.substring(0, this.fld.id.indexOf("_suggest"));
                            var c = $(d);
                            if (c) {
                                c.value = this.iHighlighted.down(".suggestInfo").innerHTML
                            }
                        }
                    }
                }
